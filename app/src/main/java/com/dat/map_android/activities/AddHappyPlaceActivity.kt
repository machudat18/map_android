...
class AddHappyPlaceActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mHappyPlaceDetails != null) {
            supportActionBar?.title = "Thêm vị trí"
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == GALLERY) {
                ...
                Toast.makeText(
                    this, "return :${DataMap.selectPos?.location}\n" +
                            "Lat : ${DataMap.selectPos?.latitude}\nLong :${DataMap.selectPos?.longitude}",
                    Toast.LENGTH_SHORT
                ).show()
                mLatitude = DataMap.selectPos?.latitude ?: 0.0
                mLongitude = DataMap.selectPos?.longitude ?: 0.0
                et_location.setText(DataMap.selectPos?.location)
            }
    ...
    companion object {
        ...
        private const val SELECT_LOCATION = 4
    }
}